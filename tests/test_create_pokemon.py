...
def test_create_pokemon_with_errors(create_pokemon_before_and_kill_all_after, create_pokemon_api, request_body, token, message):
    response = create_pokemon_api.post_create_pokemon(request_body=request_body, headers=token)
    assert response.response.json() == message


@pytest.mark.parametrize("request_body, token", CREATE_POKEMON_SUCCESS)
def test_create_more_five_pokemons(create_pokemon_before_and_kill_all_after, create_pokemon_api, request_body, token):
